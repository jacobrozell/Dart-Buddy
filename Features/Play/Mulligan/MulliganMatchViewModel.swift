...
@MainActor
final class MulliganMatchViewModel: ObservableObject {
    ...
    var mulliganState: MulliganState? { session?.runtime.mulliganState }
    ...
    var currentBotSkillProfile: BotSkillProfile? {
        guard let session, let gameState = session.runtime.mulliganState else { return nil }
    ...
    /// The active target segment for the pad lock-in (as a number, or nil for bull/none).
    var lockedSegmentNumber: Int? {
        guard let target = mulliganState?.currentTarget else { return nil }
        if case let .number(n) = target { return n }
        return nil
    }

    /// `true` when the active target is bull (pad should show bull key only).
    var isLockedToBull: Bool {
        mulliganState?.currentTarget == .bull
    }

    var activeTarget: MulliganSegment? { mulliganState?.currentTarget }

    var targetSequence: [MulliganSegment] {
        mulliganState?.config.targetSequence ?? []
    }

    var currentTargetIndex: Int {
        mulliganState?.currentTargetIndex ?? 0
    }

    var drawnTargetsAccessibilityLabel: String {
        let labels = targetSequence.map(\.displayLabel).joined(separator: ", ")
        return L10n.format("play.mulligan.drawnTargets.listAccessibilityFormat", labels)
    }

    var activeTargetAnnouncement: String {
        guard let target = activeTarget else { return "" }
        return L10n.format("play.mulligan.activeTargetFormat", target.displayLabel)
    }

    var scoreboardRows: [MulliganScoreboardView.Row] {
        guard let session, let gameState = session.runtime.mulliganState else { return [] }
    ...
    func onAppear() async {
        logger.matchInfo(
            ...
            message: "Mulligan match screen presented."
        )
    ...
    func announceTurnIfNeeded(targetAdvanced: Bool) {
        if targetAdvanced, let target = activeTarget {
            postAccessibilityAnnouncement(
                L10n.format("play.mulligan.targetAdvanced", target.displayLabel)
            )
        } else {
            guard let gameState = mulliganState else { return }
            ...
            let announcement = L10n.format(
                ...
                currentPlayer.marksOnActiveTarget
            )
    ...
    private func playSingleBotTurnIfNeeded() async -> Bool {
        guard let profile = currentBotSkillProfile,
              state == .readyTurn,
              isBotPlaying == false,
              let gameState = session?.runtime.mulliganState,
              let activeTarget = gameState.currentTarget else { return false }
        ...
        let marksAlready = gameState.players[currentPlayerIndex].marksOnActiveTarget

        var rng = SystemRandomNumberGenerator()
        let plannedDarts = DartBotEngine.generateMulliganTurn(
            activeTarget: activeTarget,
            marksAlreadyOnTarget: marksAlready,
            profile: profile,
            rng: &rng
        )
    ...
    private func submitTurnAsync(fromBotPlayback: Bool = false) async {
        ...
        guard let current = session else {
            state = .error("mulligan.error.sessionMissing")
            return
        }
        state = .submittingTurn
        let darts = enteredDarts
        let targetIndexBefore = current.runtime.mulliganState?.currentTargetIndex ?? 0

        let outcome = await turnSubmitter.submitTurn(
            from: current,
            invalidTurnFallbackKey: "mulligan.error.invalidTurn"
        ) {
            try MatchLifecycleService.submitMulliganTurn(session: current, darts: darts)
        }
        ...
        case let .succeeded(updated):
            session = updated
            let targetIndexAfter = updated.runtime.mulliganState?.currentTargetIndex ?? 0
            let didAdvance = targetIndexAfter > targetIndexBefore
            announceTurnIfNeeded(targetAdvanced: didAdvance)
    ...
    private func undoLastTurnAsync() async {
        ...
        do {
            ...
            state = .error(MatchTurnSupport.errorMessageKey(for: error, fallback: "mulligan.error.undoFailed"))
        }
    }
    ...
    func loadSessionIfNeeded() async {
        ...
        switch await MatchSessionLoader.load(
            ...
            sessionMissingFallbackKey: "mulligan.error.sessionMissing"
        ) {
...
extension MulliganMatchViewModel: MatchPlaySessionHost {
    ...
    var hostMatchType: MatchType { .mulligan }
}
