...
@MainActor
final class Bobs27MatchViewModel: ObservableObject {
    ...
    var bobs27State: Bobs27State? { session?.runtime.bobs27State }
    ...
    var currentBotSkillProfile: BotSkillProfile? {
        guard let session, let gameState = session.runtime.bobs27State else { return nil }
    ...
    var currentPlayerScore: Int {
        guard let gameState = bobs27State,
              let player = gameState.players.first(where: { $0.playerId == gameState.currentPlayerId }) else {
            return 0
        }
        return player.score
    }

    var scoreboardRows: [Bobs27ScoreboardView.Row] {
        guard let session, let gameState = session.runtime.bobs27State else { return [] }
        let isInProgress = session.runtime.status == .inProgress
        let maxScore = gameState.players.map(\.score).max() ?? 0
        return gameState.players.enumerated().map { index, player in
            let participant = session.runtime.participant(for: player.playerId)
            let isActive = index == gameState.currentPlayerIndex && isInProgress && player.isActive
            let isLeading = maxScore > 0 && player.score == maxScore
                && gameState.players.filter({ $0.score == maxScore }).count == 1
            return Bobs27ScoreboardView.Row(
                id: player.playerId,
                name: participant?.displayNameAtMatchStart ?? MatchConfigText.playerName(forIndex: index),
                score: player.score,
                isActive: isActive,
                isLeading: isLeading,
                isEliminated: player.bustOut,
                colorToken: participant?.colorToken ?? PlayerColorToken.defaultForPlayer(id: player.playerId)
            )
        }
    }

    var isBullRound: Bool {
        guard let target = bobs27State?.currentTarget else { return false }
        if case .bull = target { return true }
        return false
    }

    var lockedSegment: Int? {
        guard let target = bobs27State?.currentTarget else { return nil }
        if case let .double(segment) = target { return segment }
        return nil
    }

    var headerText: String {
        guard let state = bobs27State else { return "" }
        let roundNumber = min(state.roundIndex + 1, Bobs27State.totalRounds)
        switch state.currentTarget {
        case let .double(segment):
            return L10n.format("play.bobs27.round.doubleFormat", segment, roundNumber, Bobs27State.totalRounds)
        case .bull:
            return L10n.format("play.bobs27.round.bull", roundNumber, Bobs27State.totalRounds)
        }
    }

    var scoreText: String {
        L10n.format("play.bobs27.scoreFormat", currentPlayerScore)
    }

    var headerAccessibilityLabel: String {
        [L10n.string("play.bobs27.navTitle"), headerText, scoreText].joined(separator: ", ")
    }

    var statusBannerKey: String? {
        guard let state = bobs27State else { return nil }
        if state.isComplete { return "play.bobs27.completed" }
        if let current = state.players.first(where: { $0.playerId == state.currentPlayerId }), current.bustOut {
            return "play.bobs27.gameOver"
        }
        return nil
    }
    ...
    func onAppear() async {
        logger.matchInfo(
            ...
            message: "Bob's 27 match screen presented."
        )
    ...
    private func playSingleBotTurnIfNeeded() async -> Bool {
        guard let profile = currentBotSkillProfile,
              state == .readyTurn,
              isBotPlaying == false,
              let gameState = session?.runtime.bobs27State else { return false }
        ...
        let plannedDarts = DartBotEngine.generateBobs27Turn(
    ...
    func abandonMatch() async {
        ...
        do {
            ...
            logger.matchError(
                ...
                eventName: "bobs27_abandon_failed",
    ...
    func syncMultiplierForRound() {
        selectedMultiplier = isBullRound ? .single : .double
    }
    ...
    private func submitTurnAsync(fromBotPlayback: Bool = false) async {
        ...
        guard let current = session else {
            state = .error("bobs27.error.sessionMissing")
            return
        }
        ...
        let outcome = await turnSubmitter.submitTurn(
            from: current,
            invalidTurnFallbackKey: "bobs27.error.invalidTurn"
        ) {
            try MatchLifecycleService.submitBobs27Turn(session: current, darts: darts)
        }
