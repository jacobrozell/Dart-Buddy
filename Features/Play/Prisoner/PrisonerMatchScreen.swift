...
struct PrisonerMatchScreen: View {
    @Environment(\.horizontalSizeClass) private var horizontalSizeClass
    @ObservedObject var viewModel: PrisonerMatchViewModel
    ...
    var body: some View {
        VStack(spacing: 0) {
            MatchGameplayHeader(onExit: { showExitConfirmation = true }) {
                VStack(alignment: .leading, spacing: 2) {
                    BrandMatchScreenTitle(title: "play.prisoner.navTitle")
                    Text(viewModel.headerText)
                        .font(.caption)
                        .foregroundStyle(Brand.textSecondary)
                }
                .accessibilityIdentifier("prisoner_match_header")
            } trailing: {
                Button {
                    actionTask?.cancel()
                ...
                .accessibilityIdentifier("prisoner_undo")
            }

            if let gameState = viewModel.prisonerState {
                SideBySideMatchBody(playerCount: gameState.players.count) {
                    VStack(spacing: DS.Spacing.s3) {
                        PrisonerTargetCardView(
                            target: gameState.currentPlayer.currentTarget,
                            dartsAvailable: gameState.dartsAvailableThisVisit,
                            stuckOnBoard: gameState.currentPlayer.stuckOnBoard
                        )
                        PrisonerPrisonersListView(rows: viewModel.prisonerRows)
                        PrisonerScoreboardView(rows: viewModel.scoreboardRows)
                    }
                } padChrome: {
                    stateBanner
                } controls: {
        ...
        .sheet(isPresented: $viewModel.showRingPicker) {
            if let segment = viewModel.pendingRingSegment {
                PrisonerSingleRingPickerView(segment: segment) { hit in
                    viewModel.confirmRingHit(hit)
                }
            }
        }
    ...
    private var prisonerPad: some View {
        DartNumberPad(
        ...
        .accessibilityHint(
            viewModel.canHumanInput
                ? L10n.string("play.prisoner.pad.hint")
                : L10n.string("play.prisoner.pad.disabledWhileBot")
        )
    }
