...
struct SuddenDeathMatchScreen: View {
    @Environment(\.horizontalSizeClass) private var horizontalSizeClass
    @ObservedObject var viewModel: SuddenDeathMatchViewModel
        ...
        VStack(spacing: 0) {
            MatchGameplayHeader(onExit: { showExitConfirmation = true }) {
                VStack(alignment: .leading, spacing: 2) {
                    BrandMatchScreenTitle(title: "play.suddenDeath.navTitle")
                    HStack(spacing: DS.Spacing.s2) {
                        Text(viewModel.headerText)
                            .font(.caption)
                            .foregroundStyle(Brand.textSecondary)
                        Text(viewModel.playersRemainingText)
                            .font(.caption)
                            .foregroundStyle(Brand.textSecondary)
                    }
                }
                ...
                .accessibilityIdentifier("suddenDeath_match_header")
            } trailing: {
                Button {
                    actionTask?.cancel()
                ...
                .accessibilityIdentifier("suddenDeath_undo")
            }
                    ...
                    VStack(spacing: DS.Spacing.s3) {
                        ...
                        if let eliminationText = viewModel.lastRoundEliminationText {
                            Text(eliminationText)
                                ...
                                .accessibilityIdentifier("suddenDeath_elimination_banner")
                        }
                    }
                } padChrome: {
                    stateBanner
    ...
    private var stateBanner: some View {
        ...
        case .eliminationFeedback:
            MatchFeedbackBanner(text: "play.suddenDeath.eliminatedThisRound", style: .legWin)
                .accessibilityHidden(true)
                .accessibilityIdentifier("suddenDeath_elimination_feedback")
        default:
            EmptyView()
        }
    }

    private var suddenDeathPad: some View {
        DartNumberPad(
