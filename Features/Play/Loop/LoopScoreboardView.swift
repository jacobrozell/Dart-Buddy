...
struct LoopScoreboardView: View {
        ...
        VStack(spacing: DS.Spacing.s2) {
            ForEach(rows) { row in
                HStack(spacing: DS.Spacing.s2) {
                    ...
                    Spacer()
                    if row.isEliminated {
                        Text(L10n.string("play.loop.playerEliminated"))
                            .font(.caption.weight(.semibold))
                            .foregroundStyle(Brand.red)
                    } else {
                        LoopLivesPipsView(total: row.startingLives, remaining: row.lives)
                    }
                }
...
private struct LoopLivesPipsView: View {
    let total: Int
    let remaining: Int
    ...
    var body: some View {
        ...
        .accessibilityLabel(L10n.format("play.loop.livesRemainingFormat", remaining))
    }
}
