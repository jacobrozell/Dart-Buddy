...
struct HalveItMatchScreen: View {
    @Environment(\.horizontalSizeClass) private var horizontalSizeClass
    @ObservedObject var viewModel: HalveItMatchViewModel
        ...
        VStack(spacing: 0) {
            MatchGameplayHeader(onExit: { showExitConfirmation = true }) {
                VStack(alignment: .leading, spacing: 2) {
                    BrandMatchScreenTitle(title: "play.halveIt.navTitle")
                ...
                .accessibilityIdentifier("halve_it_match_header")
            } trailing: {
                Button {
                    actionTask?.cancel()
                ...
                .accessibilityIdentifier("halve_it_undo")
            }

            if let gameState = viewModel.halveItState {
                SideBySideMatchBody(playerCount: gameState.players.count) {
                    HalveItScoreboardView(
                        rows: viewModel.scoreboardRows,
                        targetSegment: gameState.currentTarget
                    )
                } padChrome: {
                    stateBanner
                } controls: {
    ...
    private var halveItPad: some View {
        DartNumberPad(
        ...
        .accessibilityHint(
            viewModel.canHumanInput
                ? (lockedSegmentHint ?? "")
                : L10n.string("play.halveIt.pad.disabledWhileBot")
        )
    }

    private var lockedSegmentHint: String? {
        guard let segment = viewModel.lockedSegment else { return nil }
        return L10n.format("play.halveIt.pad.lockedSegmentHint", segment)
    }
