...
struct MickeyMouseMatchScreen: View {
    @Environment(\.horizontalSizeClass) private var horizontalSizeClass
    @ObservedObject var viewModel: MickeyMouseMatchViewModel
        ...
        VStack(spacing: 0) {
            MatchGameplayHeader(onExit: { showExitConfirmation = true }) {
                VStack(alignment: .leading, spacing: 2) {
                    BrandMatchScreenTitle(title: "play.mickeyMouse.title")
                ...
                .accessibilityIdentifier("mickeyMouse_match_header")
            } trailing: {
                Button {
                    actionTask?.cancel()
                ...
                .accessibilityIdentifier("mickeyMouse_undo")
            }
            ...
            if let gameState = viewModel.mickeyMouseState {
                SideBySideMatchBody(playerCount: gameState.players.count) {
                    ...
                    .accessibilityIdentifier("mickeyMouse_markBoard")
                } padChrome: {
                    stateBanner
                } controls: {
                    mickeyMousePad
    ...
    private var stateBanner: some View {
        ...
        case .targetAdvanced:
            MatchFeedbackBanner(text: "play.mickeyMouse.announce.targetAdvanced", style: .cricketClosure)
                .accessibilityHidden(true)
                .accessibilityIdentifier("mickeyMouse_targetAdvancedBanner")
        default:
            EmptyView()
        }
    }

    private var mickeyMousePad: some View {
        DartNumberPad(
            enteredDarts: $viewModel.enteredDarts,
            selectedMultiplier: $viewModel.selectedMultiplier,
            lockedSegment: viewModel.lockedSegment,
            showsBull: viewModel.activeTarget == .bull,
            onUndoTurn: {
                actionTask?.cancel()
                actionTask = Task { await viewModel.undoLastDart() }
            }
        ...
        .accessibilityHint(
            viewModel.canHumanInput
                ? padLockedHint ?? ""
                : L10n.string("play.mickeyMouse.pad.disabledWhileBot")
        )
    }

    private var padLockedHint: String? {
        guard let target = viewModel.activeTarget else { return nil }
        return L10n.format("play.mickeyMouse.pad.lockedTargetHint", target.displayLabel)
    }
