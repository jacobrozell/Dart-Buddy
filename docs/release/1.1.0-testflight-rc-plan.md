...
| **Raid** (co-op) | Other co-op (Cerberus, Vault, Clear the Board, …) |
| Around the Clock (practice) | Other practice modes |
...
**Acceptance:** Release build cannot start Golf, Fleet, Cerberus, Mickey Mouse, or 180 ATC. **Raid must be reachable.**
...
Update or add **`PartyPack1_1SmokeUITests`** (replaces retired `Lean1_0SmokeUITests`):

- 4 tabs, no Modes
- Picker lists exactly **7 modes**
- Start match: Baseball, Killer (3p), Shanghai, **Raid**, Around the Clock
- Hidden: Golf, Fleet, Training Partner, export
...
- `ProductSurfaceTests` — Raid + Around the Clock reachable; Golf/Fleet not
...
### 2.2 Lean UI suite on release branch

Run **`DartBuddyUILean`** locally on `release/*` before shipping (Party Pack / `ProductSurface` smoke).
