...
- Waves 1–4 (`de`, `es`, `nl`, `fr`) — shipped; see §2.
...
- PRs adding keys must update **all** shipped locale files (`en` + `de` + `es` + `nl` + `fr`).
...
## 8. Testing
- `LocalizationParityTests` (`.localization`): en/de/es/nl/fr key set and `%@`/`%d`/etc. parity.
...
- Manual language switch smoke tests across tabs and gameplay flows (Simulator → Deutsch / Español / Nederlands / Français).
