...
- **Wave 4 (shipped):** Italian (`it`) — system locale only; no in-app language picker.
...
- `Resources/it.lproj/Localizable.strings` (Italian wave 4)
...
- Waves 1–4 (`de`, `es`, `nl`, `it`) — shipped; see §2.
...
- PRs adding keys must update **all** shipped locale files (`en` + `de` + `es` + `nl` + `it`).
...
## 8. Testing
- `LocalizationParityTests` (`.localization`): en/de/es/nl/it key set and `%@`/`%d`/etc. parity.
...
- Manual language switch smoke tests across tabs and gameplay flows (Simulator → Deutsch / Español / Nederlands / Italiano).

### App Store Connect (localized listings, manual)
- Localized subtitle, description, and keywords per shipped locale.
- Screenshots captured with device language set to Deutsch, Español, Nederlands, or Italiano as appropriate.
