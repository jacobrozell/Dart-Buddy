...
App Store assets for **Baseball**, **Killer**, **Shanghai**, **Raid**, and **Around the Clock**.
...
# Portrait only (faster App Store pass)
...
# → marketing-screenshots/1.1/ipad/raw/*.png
...
Captures use `-enable_lean_product_surface` so the binary matches the shipping **Party Pack 1.1** allowlist (seven modes, four tabs, English UI). `-skip_release_highlights` suppresses the in-app What's New sheet.

## Screens captured

| File slug | Screen | Launch args |
|-----------|--------|-------------|
| `10-raid-match` | Raid expose phase (2 heroes, boss HP visible) | `-snapshot_match_raid` |
