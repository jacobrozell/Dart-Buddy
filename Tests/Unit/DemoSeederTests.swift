...
struct DemoSeederTests {
    ...
    @Test
    func raidSnapshotLaunchArgumentStoresActiveMatch() async throws {
        let dependencies = try makeDependencies()

        await DemoSeeder.seedIfRequested(dependencies, arguments: ["-snapshot_match_raid"])

        let summary = await MainActor.run { dependencies.activeMatchStore.activeMatchSummary() }
        #expect(summary?.id.uuidString == "00000000-0000-0000-0000-000000000009")
        #expect(summary?.type == .raid)
        #expect(summary?.status == .inProgress)
    }
