...
private func makeMickeyMouseViewModel(
    ...
    let vm = MickeyMouseMatchViewModel(
        ...
        matchRepository: FakeMatchRepositoryBuilder.matchViewModel(completedType: .mickeyMouse),
...
func mickeyMouseViewModelRehydratesSessionFromSnapshotWhenStoreEmpty() async throws {
    ...
    let vm = MickeyMouseMatchViewModel(
        ...
        store: store,
        logger: DefaultAppLogger(minimumLevel: .fault, sink: TestNoopLogSink()),
        matchRepository: FakeMatchRepositoryBuilder.rehydrating(snapshot: snapshotSummary, completedType: .mickeyMouse),
