...
enum DemoSeeder {
    ...
    static func seedIfRequested(_ dependencies: AppDependencies, arguments: [String]) async {
        ...
        if arguments.contains("-snapshot_match_raid") {
            await seedRaidSnapshot(dependencies)
        }
    ...
    /// Populates the fixed Raid match id used by `-snapshot_match_raid`
    /// (Jacob + Sam in expose phase; boss at 34 HP after three shield closes).
    private static func seedRaidSnapshot(_ dependencies: AppDependencies) async {
        let matchId = UUID(uuidString: "00000000-0000-0000-0000-000000000009") ?? UUID()
        let config = MatchConfigPayload.raid(
            MatchConfigRaid(bossTier: .standard, heroHearts: 3, enrageEnabled: false)
        )
        let participants = [
            MatchParticipant(playerId: UUID(), displayNameAtMatchStart: "Jacob", turnOrder: 0),
            MatchParticipant(playerId: UUID(), displayNameAtMatchStart: "Sam", turnOrder: 1)
        ...
        do {
            ...
            session = try MatchLifecycleService.submitRaidVisit(
                session: session,
                darts: [d(.triple, 20), d(.single, 20), d(.single, 20)]
            )
            session = try MatchLifecycleService.submitRaidVisit(
                session: session,
                darts: [d(.triple, 19), d(.single, 19), d(.single, 19)]
            )
            session = try MatchLifecycleService.submitRaidVisit(
                session: session,
                darts: [d(.triple, 18), d(.single, 18), d(.single, 18)]
            )
            session = try MatchLifecycleService.submitRaidVisit(
                session: session,
                darts: [d(.double, 16), d(.single, 16), d(.single, 16)]
            )
            let finalSession = session
            await MainActor.run { dependencies.activeMatchStore.save(finalSession) }
        } catch {
            dependencies.logger.error(
                .appLifecycle,
                eventName: "raid_snapshot_seed_failed",
                message: "Raid snapshot seed failed: \(error)"
            )
        }
    }
